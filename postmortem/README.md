...
## Analysis of poly(A)-tail containing reads in NYGC dataset (reviewer only)

Compare the recovery of PATRs from the NYGC dataset to in vitro TDP-43 KD samples and in vitro SLAM-seq samples.

Requires the sample tables used as input to and the 'all_samples.patrs.valid_stats.tsv', pooled and per-sample 'PAS cluster' BED files generated by my PATR extraction pipeline [bulk_polyatail_reads](https://github.com/SamBryce-Smith/bulk_polyatail_reads/tree/v0.3.0) (v0.3.0).

`scripts/intersect_bed_ref_by_dir.sh` uses `bedtools window` to compute the overlap of per sample PATR PAS cluster with PAS from the PolyASite 2.0 database. A window size of 12 was used. This was run on per-sample BEDs for all three datasets, generating the `*.polyasite_overlap.wcounts.tsv` files.

`scripts/nygc_patrs.R` performs visualisation of global PATR statistics for the NYGC dataset, comparing against in vitro TDP-43 KD samples from this dataset and counts from a SLAM-seq experiment (also Ribo-depletion).
