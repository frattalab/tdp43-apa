...
# add gene names for enriched cryptics
gn_dot_plot <- base_dot_plot +
...
ggsave(filename = "processed/gene_exprn/2023-09-12_cryptic_gene_exprn_dot_plot_no_label.png",
       plot = base_dot_plot,
       height = 8,
       width = 8,
       units = "in",
...
ggsave(filename = "processed/gene_exprn/2023-09-12_cryptic_gene_exprn_dot_plot_gn_label.png",
       plot = gn_dot_plot,
       height = 8,
       width = 8,
       units = "in",
...
### Summary plots of gene expression for enriched vs not erniched cryptics

plot_liu_all_means <- liu_all_means %>%
  filter(abundance_type == "counts_norm") %>%
  mutate(gene_name = fct_reorder(gene_name, mean_all),
         enr_cryp = if_else(gene_name %in% cryp_enriched_gn, T, F),
         plot_mean = log2(mean_all + 1)) 

cryp_enr_vs_ge_density <- plot_liu_all_means %>%
  ggplot(aes(x = plot_mean, fill = enr_cryp, colour = enr_cryp)) +
...
cryp_enr_vs_ge_box <- plot_liu_all_means %>%
  ggplot(aes(y = plot_mean, x = enr_cryp, colour = enr_cryp)) +
...
ggsave(filename = "processed/gene_exprn/2023-09-12_cryptic_gene_exprn_enr_vs_not_density_plot.png",
       plot = cryp_enr_vs_ge_density,
       height = 8,
       width = 8,
       units = "in",
       dpi = "retina")

ggsave(filename = "processed/gene_exprn/2023-09-12_cryptic_gene_exprn_enr_vs_not_boxplot.png",
       plot = cryp_enr_vs_ge_box,
       height = 8,
       width = 8,
       units = "in",
       dpi = "retina")
