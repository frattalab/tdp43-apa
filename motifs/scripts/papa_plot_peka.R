...
# plot just GU 6mer
all_map_gu <- plot_kmer_dbrn(mutate(dbrn_tbl_long_gu, direction = "cryptic"),
               rolling_mean = T,
...
# plot just UG 6mer
all_map_ug <- plot_kmer_dbrn(mutate(dbrn_tbl_long_ug, direction = "cryptic"),
               rolling_mean = T,
               facet_w = "plot_event_type ~ plot_region_type",
               n_col = 2,
               n_row = 3,
...
all_map_both <- plot_kmer_dbrn(mutate(dbrn_tbl_long_both, direction = "cryptic"),
               rolling_mean = T,
               facet_w = "plot_event_type ~ plot_region_type",
               n_col = 2,
               n_row = 3,
...
plots_top10_comparison <- map(dbrn_tbl_top10_comparison_list,
    ~ plot_kmer_dbrn(.x,
                    rolling_mean = T,
                    facet_w = "plot_event_type ~ plot_region_type",
                    n_col = 2,
...
pdf("processed/peka/papa/plots/2023-11-14_papa_all_comparisons_gu_ug_map.pdf", width = a4_height, height = a4_width )

# Loop through each ggplot object and print it to the PDF file
plots_ug_list <- list(all_map_ug, all_map_gu, all_map_both, all_map_both_nosum)
...
# top 10 kmers plots
pdf("processed/peka/papa/plots/2023-11-14_papa_peka_top10_kmers_by_comparison_map.pdf", width = a4_height, height = a4_width )
