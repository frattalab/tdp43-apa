...
path_sample_tbl <- "data/fracseq/ritter_short_read_fracseq_sample_sheet_minus_esc_hp_rep3.csv"

# opt_parser$sample_table
sample_tbl <- read.table(path_sample_tbl,
                         header = TRUE,
                         sep = ",",
...
output_prefix <- file.path("processed", "fracseq", "2024-04-30_summarised_pas")
...
sample_names <-  paste(sample_tbl$unit, sample_tbl$sample_name, sep = "_")
...
salmon_dir <- "data/fracseq/salmon_papa_index"

quant_paths <- file.path(salmon_dir, sample_names, "quant.sf")
...
#2 read in tx2pas
tx2le <- "../postmortem/processed/2023-06-22_cryptics_plus_decoys.decoys_full_fix_tx2le.tx2le.tsv"
le2gene <- "../postmortem/processed/2023-06-22_cryptics_plus_decoys.decoys_full_fix_tx2le.le2gene.tsv"

tx2pas <- read.table(tx2le,
                     header = T,
                     sep = "\t",
                     stringsAsFactors = FALSE)

tx2gene <- read.table(le2gene,
                      header = T,
                      sep = "\t",
                      stringsAsFactors = FALSE)

elk1_tx2pas <- tx2pas %>%
  filter(str_detect(le_id, "ENSG00000126767"))

elk1_tx2gene <- tx2gene %>%
  filter(str_detect(le_id, "ENSG00000126767"))

txi.tx <- tximport(quant_paths,
                   type = "salmon",
                   txOut = TRUE,
                   tx2gene = elk1_tx2gene, # doesn't matter what this is, but neeeds to be populated
                   countsFromAbundance = "lengthScaledTPM")

View(txi.tx$abundance)
View(txi.tx$infReps)
...
txi.pas.inf <- summarizeToGene(txi.tx, tx2pas, countsFromAbundance = "lengthScaledTPM", varReduce = T)

View(txi.pas$counts)
View(txi.pas$infReps)
View(txi.pas.inf$counts)
View(txi.pas.inf$variance)
# print(head(txi.pas$abundance))
# print(head(txi.pas$counts))
# To check: are row idxs of inferential rep matrices consistent with the counts matrix?
...
iso2gene <- merge(tx2gene, tx2pas, by = isoform_id_col)[,c("gene_id", isoform_id_col)]
# print(head(iso2gene))
...
if (!dir.exists(dirname(output_prefix))) {
  ...
  dir.create(dirname(output_prefix),recursive = T)
  }
...
message(glue("Writing per-event PPAU matrix to {output_prefix}.ppau.tsv"))
write.table(ppau,
...
message(glue("Writing per-gene TPM matrix to {output_prefix}.gene_tpm.tsv"))
